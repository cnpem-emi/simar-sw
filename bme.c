...
void get_open_iter(struct sensor_data* sensor)
{
    ...
    */
    ...
    // minutes
    if (sensor->average - sensor->data.pressure < -0.23 || (sensor->open_average != 0 && sensor->open_average - sensor->data.pressure < 0.23)) {
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (bme_init(&sensor.dev,
            ...
            if (valid_i < argc)
                ...
                snprintf(sensors[valid_i].name, MAX_NAME_LEN, "sensor_%d_%x", i % 4, i < 4 ? BME280_I2C_ADDR_PRIM : BME280_I2C_ADDR_SEC);
    ...
    do {
        ...
        if (c_remote->err) {
            if (server_i == 0) {
                syslog(LOG_ERR, "No remote Redis server instance for calibration is available. Attempting to fetch local mirror.\n");
                c_remote = c;
            }
        ...
        if (!reply->str) {
            ...
            for (int j = 0; j < WINDOW_SIZE; j++) {
                ...
                if (check_alteration(sensors[i])) {
                    ...
                    if (retries > 10) {
                        syslog(LOG_ERR,
                            "Could not obtain realistic data from sensor number %d\n",
                            i);
                        return SENSOR_FAIL;
                    }
                }
    ...
    for (;;) {
        for (i = 0; i < valid_i; i++) {
            ...
            if (check_alteration(sensors[i])) {
                ...
                reply = (redisReply*)redisCommand(
                    c, "SET %s_%s %d", "open", sensors[i].name, sensors[i].is_open);
                freeReplyObject(reply);

                reply = (redisReply*)redisCommand(
                    c, "SET %s_%s %.3f", "avg", sensors[i].name, sensors[i].average);
                freeReplyObject(reply);
        ...
        if (reply_remote->str) {
            reply = (redisReply*)redisCommand(
                c, "SET last_ext_pressure %s", reply_remote->str);
            freeReplyObject(reply);
        }
