...
class DAC:
    def __init__(self):
        ...
        spicon.transfer(spi, b"\x35\x96\x30")
        GPIO.output(DAC_pin, GPIO.HIGH)

        GPIO.output(DAC_pin, GPIO.LOW)
        spicon.transfer(spi, b"\x20\x00\x00")
            ...
            if type(data) is list:
                for index, value in enumerate(channels):
                    ...
                    spicon.transfer(
                        spi,
                        bytes(
                            [0xB0 + value, data[index] >> 4, (data[index] << 4) & 0xFF]
                        ),
                    )
                    GPIO.output(DAC_pin, GPIO.HIGH)
                ...
                for value in channels:
                    GPIO.output(DAC_pin, GPIO.LOW)
                    spicon.transfer(
                        spi, bytes([0xB0 + value, data >> 4, (data << 4) & 0xFF])
                    )
                    GPIO.output(DAC_pin, GPIO.HIGH)
        else:
            GPIO.output(DAC_pin, GPIO.LOW)
            spicon.transfer(
                spi, bytes([0xB0 + channels, data >> 4, (data << 4) & 0xFF])
            )
            GPIO.output(DAC_pin, GPIO.HIGH)

    def WD_RESET(self):
        GPIO.output(DAC_pin, GPIO.LOW)
        spicon.transfer(spi, b"\x33\x96\x30")
        GPIO.output(DAC_pin, GPIO.HIGH)
        return
...
class ADC:
    ...
    def read(self, channels):
        ...
        if type(channels) is list:
            ...
            GPIO.output(ADC_pin, GPIO.LOW)
            spicon.transfer(spi, bytes([0x83 + channels[0] * 4, 0x10]))
            GPIO.output(ADC_pin, GPIO.HIGH)

            for index in range(len(channels)):
                GPIO.output(ADC_pin, GPIO.LOW)
                _data = spicon.transfer(
                    spi, bytes([0x83 + _channels[index + 1] * 4, 0x10])
                )
                GPIO.output(ADC_pin, GPIO.HIGH)
            ...
            GPIO.output(ADC_pin, GPIO.LOW)
            spicon.transfer(spi, bytes([0x83 + channels * 4, 0x10]))
            GPIO.output(ADC_pin, GPIO.HIGH)

            GPIO.output(ADC_pin, GPIO.LOW)
            _data = spicon.transfer(spi, bytes([0x83 + channels * 4, 0x10]))
            GPIO.output(ADC_pin, GPIO.HIGH)
