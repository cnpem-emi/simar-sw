...
int main(int argc, char* argv[]) {
  ...
  while (1) {
    ...
    if (check_alteration(sensor)) {
      syslog(LOG_DEBUG, "%ld,%.3f,%.3f,%.3f", (unsigned long)time(NULL), sensor.data.temperature,
             sensor.data.pressure, sensor.data.humidity);
      reply = (redisReply*)redisCommand(c, "SET %s%s %.3f", "temperature", sensor.name,
                                        sensor.data.temperature);
      freeReplyObject(reply);

      reply = (redisReply*)redisCommand(c, "SET %s%s %.3f", "pressure", sensor.name,
                                        sensor.data.pressure);
      freeReplyObject(reply);

      reply = (redisReply*)redisCommand(c, "SET %s%s %.3f", "humidity", sensor.name,
