...
install_wireless: install_common
	sed -i -e "9iExecStartPre=/bin/sleep 8 " ./start/services/simar@.service
	sed -i -e "9iExecStartPre=/bin/sh -c 'echo none > /sys/class/leds/beaglebone\:green\:usr3/trigger'" ./start/services/simar@.service
	sed -i -e "9iExecStartPre=/bin/sh -c 'echo none > /sys/class/leds/beaglebone\:green\:usr2/trigger'" ./start/services/simar@.service
	cp ./start/services/simar@.service /etc/systemd/system/.
