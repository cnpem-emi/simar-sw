...
static PyObject* count_pru(PyObject* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "Ih", &time_base, &pru))
    return NULL;
  ...
  if (read(pfd.fd, readBuf, MAX_BUFFER_SIZE)) {
    ...
    uint32_t count = 0;
    for (int i = 0; i < 4; i++) {
      count = (readBuf[offset + 3] << 24) | (readBuf[offset + 2] << 16) |
              (readBuf[offset + 1] << 8) | readBuf[offset];
...
static struct PyModuleDef countingpru_module = {
    PyModuleDef_HEAD_INIT, "countingpru", "Python interface for CountingPRU's counting function",
    -1, CountMethod};
