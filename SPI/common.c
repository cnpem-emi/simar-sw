...
int bbb_mmio_get_gpio(gpio_t *gpio)
{
    ...
    if (gpio_base[base] == NULL)
    {
        ...
        gpio_base[base] = (uint32_t *)mmap(NULL, GPIO_LENGTH, PROT_READ | PROT_WRITE, MAP_SHARED, mfd, gpio_addresses[base]);
...
int spi_mod_comm(char *tx, char *rx, int len)
{
    ...
    if (_mode != 3) ioctl(fd, SPI_IOC_WR_MODE, &mod_mode);
    if (_bits != 8) ioctl(fd, SPI_IOC_WR_BITS_PER_WORD, &mod_bits);

    // Reusing device file descriptor, but creating custom struct to prevent confusion
    struct spi_ioc_transfer tr = {
        .tx_buf = (unsigned long)tx,
        .rx_buf = (unsigned long)rx,
        .len = len,
    ...
    if (_mode != 3) ioctl(fd, SPI_IOC_WR_MODE, &_mode);
    if (_bits != 8) ioctl(fd, SPI_IOC_WR_BITS_PER_WORD, &_bits);
