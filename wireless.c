...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < 20; i++) {
        ...
        if(!local_c->err) break;

        sensor.dev.delay_us(500000, NULL);
    }

    if(local_c->err) syslog(LOG_CRIT, "Could not find a local Redis server");
    ...
    for(int i = 0; i < 10; i++) bme_read(&sensor.dev, &sensor.data); // Perform "calibration" readings
    ...
    for (;;) {
        ...
        if (check_alteration(sensor)) {
            ...
            if(strcmp(filename, "")) {
                ...
                fprintf(file, "%s,%.4f,%.4f,%.4f\n", time_str, sensor.data.temperature, sensor.data.pressure, sensor.data.humidity);
                fflush(file);
            }
        }
